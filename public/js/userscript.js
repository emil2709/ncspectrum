...
function checkinCheck()
{
  ...
  if((listlength <= 0) && (users.length <= 0))
  {
 ...
 * The users and visitors array will be updated by removing the userid from the arrays, along with updating
 * the counter which acts as both a counter for number of users checkedin and as an index for the arrays.
 */
function checkout()
{
  var users = JSON.parse(sessionStorage.users);
 ...
 * AJAX will send all the users saved in the temporary frontend array to the temporary backend array
 * so it can find the saved users from the database. 
 * This will upon success direct the user to the visit creation page.
 ...
 * 
 * This function is called when the Checkin submit button is clicked, when a visit is being created.
 * This function will transfer the users in the temporary users array over to the visitors array.
 * NB!
 * Users array is an temporary array that contains all the users that are checking in and out,
 * but have yet to actually create a visit.
 * Visitors array is a final array that contains all the users that has created a visit.
 */
 ...
 * The purpose of this script is to check if the frontend visitors array match with the backend visitor array.
 * If there is a missmatch where the backend array is shorter than the frontend array, that
 * probably means an administrator have manually checked someone out and will proceed to
 * fetch the up-to-date array from the backend and update the frontend arrays before refreshing the page.
...
function autosync()
{
  ...
  $.ajax({
    ...
    success: function(data){
      // If there is a missmatch between backend and frontend arraylength.
      if(data.length < visitors.length)
      {
        // Finds the differences between the visitors and data array.
        difference = $.grep(visitors,function(x) {return $.inArray(x, data) < 0});
        // Finds the difference and deletes them from the users array.
        for(var i = 0; i < difference.length; i++)
        {
          for(var j = 0; j < users.length; j++)
          {
            if(users[j] == difference[i])
            {
              users.splice(j,1);
              counter--;
            }
        ...
        // Updates the session variables with new values.
        sessionStorage.users = JSON.stringify(users);
